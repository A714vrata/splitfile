...
int main(int argc, char* argv[]) {
  ...
  if (argc < 2) {
    fprintf(stderr, "Error: Missing argument. Usage %s <filename>\n", argv[0]);
    return EXIT_FAILURE;
  } else if (argc > 2) {
    fprintf(stderr, "Error: Too many arguments. Usage %s <filename>\n",
            argv[0]);
    return EXIT_FAILURE;
  }
  ...
  while (!feof(readpoint)) {
    ...
    if (writepoint == NULL) {
      ...
      fprintf(writepoint, "%s",
              txbuf);  // Write file buffer to split file "section"
    }
    fclose(writepoint);
    runs++;
  }
